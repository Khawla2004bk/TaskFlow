<?php
class DatabaseConfig {
    // Utilisation de constantes pour les paramètres de connexion
    private const HOST = 'localhost';
    private const USERNAME = 'root';
    private const PASSWORD = '';
    private const DBNAME = 'taskflow';

    // Singleton pour éviter les connexions multiples
    private static ?PDO $instance = null;

    // Constructeur privé pour empêcher l'instanciation directe
    private function __construct() {}

    public static function getConnection(): PDO {
        // Vérification de l'existence d'une instance de connexion
        if (self::$instance === null) {
            try {
                // Configuration sécurisée de la connexion
                $dsn = "mysql:host=" . self::HOST . ";dbname=" . self::DBNAME . ";charset=utf8mb4";
                $options = [
                    PDO::ATTR_ERRMODE => PDO::ERRMODE_EXCEPTION,
                    PDO::ATTR_DEFAULT_FETCH_MODE => PDO::FETCH_ASSOC,
                    PDO::ATTR_EMULATE_PREPARES => false, // Désactive l'émulation des requêtes préparées
                    PDO::MYSQL_ATTR_FOUND_ROWS => true
                ];

                self::$instance = new PDO($dsn, self::USERNAME, self::PASSWORD, $options);
                
                // Désactivation des fonctionnalités potentiellement dangereuses
                self::$instance->exec('SET NAMES utf8mb4');
            } catch (PDOException $e) {
                // Log de l'erreur sans révéler de détails sensibles
                error_log("Erreur de connexion à la base de données : " . $e->getMessage());
                throw new Exception("Impossible de se connecter à la base de données.");
            }
        }

        return self::$instance;
    }

    // Empêche le clonage de l'objet
    private function __clone() {}

    // Méthode pour fermer proprement la connexion
    public static function closeConnection() {
        self::$instance = null;
    }
}
?>
